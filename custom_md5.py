...
def fill(bt):
    if isinstance(bt, str):
        bt = bytes(bt, encoding='utf-8')
    bArr = [padRight(bin(i).replace("0b",""), "0", 8) for i in bt]
    bStr = "".join(bArr)
    L = len(bStr) % 512
    ...
    if L == 0:
        bStr += ENDFLAG
        bStr += R("0", 447)
        bStr += LE64(padRight(bin((len(bArr)*8)%(1<<64)).replace("0b",""), "0", 64))
    elif L < 448:
        bStr += ENDFLAG
        bStr += R("0", 447 - L)
        bStr += LE64(padRight(bin((len(bArr)*8)%(1<<64)).replace("0b",""), "0", 64))
...
def check(fic, hash):
    if md5_file(fic) == hash:
        return 'correspondent'
    return 'ne correspondent pas'